#!/usr/bin/env python3
"""
Standalone test script for reference text alignment fix v3.
Tests fixed semantic-aware alignment logic.
"""

import re


# Fixed semantic-aware version of alignment function
def align_text_to_timeline(timeline_subs, reference_text, max_chars=20):
    ...
    3. Limit to max 2 segments per time period to avoid cross-sentence concatenation
    4. Merge short segments intelligently
    """
    ...
    for i, time_seg in enumerate(timeline_subs):
        ...
        # Add at most 2 semantic segments to avoid cross-sentence concatenation
        # Only add a segment if it fits within max_chars
        while (
        ...
        # If segment is empty or too short, skip
...
def test_alignment():
    ...
    print("=" * 80)
    print("Testing FIXED Semantic-Aware Alignment")
    print("=" * 80)
    ...
    for sub in aligned_subs:
        ...
        elif len(text) < 10 and text[0] not in "看着梦想专梭哈心好我":
            # Likely fragment
            partial_sentences += 1

    print(f"   Complete sentences (ending with 。！？；：): {complete_sentences}")
    print(f"   Potential fragments: {partial_sentences}")
    if complete_sentences > len(aligned_subs) * 0.5:
        print("   ✓ Good balance of complete/partial sentences")
    else:
        print("   ⚠ Many subtitles are fragments")

    # Display full aligned text for manual inspection
    print(f"\n7. Full Aligned Text (first 400 chars):")
    print("-" * 80)
    ...
    with open("test_output_fixed.srt", "w", encoding="utf-8") as f:
        f.write(srt_output)
    print("\n✓ SRT output saved to: test_output_fixed.srt")
